...
TQDM_DISABLE = False  # Enable tqdm progress bar
...
class BertSentClassifier(torch.nn.Module):
    def __init__(self, config):
        ...
        # Add a dropout layer and a linear layer for classification
        self.dropout = torch.nn.Dropout(config.hidden_dropout_prob)
        self.classifier = torch.nn.Linear(config.hidden_size, self.num_labels)

        # Freeze or unfreeze BERT parameters based on the option
    ...
    def forward(self, input_ids, attention_mask):
        ...
        # Get the pooled output (representation of the [CLS] token)
...
def create_data(filename, flag='train'):
    ...
    with open(filename, 'r', encoding='utf-8') as fp:  # Added encoding='utf-8'
        for line in fp:
            # Split the line into label and sentence
            label, org_sent = line.strip().split(' ||| ')
            sent = org_sent.lower().strip()  # Convert to lowercase and remove extra spaces
            ...
            # Append the data as a tuple (sentence, label, tokens)
            data.append((sent, label, tokens))

    print(f"Loaded {len(data)} examples from {filename}")
    if flag == 'train':
        return data, len(num_labels)  # Return data and number of unique labels for training
    else:
        return data  # Return only data for validation/testing
...
def test(args):
    with torch.no_grad():
        ...
        with open(args.dev_out, "w+", encoding='utf-8') as f:  # Added encoding='utf-8'
            print(f"Dev Acc = {dev_acc:.3f}")
            for s, t, p in zip(dev_sents, dev_true, dev_pred):
                f.write(f"{s} ||| {t} ||| {p}\n")

        with open(args.test_out, "w+", encoding='utf-8') as f:  # Added encoding='utf-8'
